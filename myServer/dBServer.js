...
function dBServer(routes = {}){
    ...
    const server = myHttps.createServer(serverOpts, (request, response) =>{
        let parsedUrl = url.parse(request.url)//get the url and parse it
        let filename = parsedUrl.pathname
        filename = filename === '/' ? '/homepage.html' : filename //for static

        //custom routing 
        if(routes[filename]){
            routes[filename](request, response, query.parse(parsedUrl.query)) //stringify the parsed url
            return;
        }
    ...
    server.listen(8180, ()=>{
        console.log("the server started at server 8180")
    })
}
